...
// |can_scale|=|0| |id|=|1| |part|=|0| |pos|=|56,86| |rotate|=|0| |scale_x|=|1| |scale_y|=|1|
    NamedNodeMap nm = el.getAttributes();
  ...
  public void setDetails(NodeList nl) {
    ...
    for (int i = 0; i < nl.getLength(); i++) {
      ...
      if (n instanceof Element) {
        ...
        if (ModelDSN.debug >= 2) System.out.println("!!!Element: ("+el.getChildNodes().getLength()+") "+el.toString());
          if (el.getNodeName().equals("Size")) {
            details.setSize(el);
          } else
          if (el.getNodeName().equals("GUIDES")) {
